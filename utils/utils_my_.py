...
groupby_src_lookup = {
    # FQIDS (instance of class Lookup) => __main__.Lookup 
    # x_fqids_pointer (from features_generation.ipynb) will refer to the C function "fill_history" argument x_fqids.
    'fqid': ('x_fqids', 'FQIDS'),
    'name': ('x_n', 'NAMES'),
...
value_src_lookup = {
    # et_since_prev => elapsed_time since previous
    # x_index_pointer (from features_generation.ipynb) will refer to the C function "fill_history" argument x_index.
...
# add simple filtered agg featues. e.g., LG0__LNone__fqid__tomap__ETsinceprev__min
FEATURE_DEFS.extend([
...
def get_features_info_container():
    c = defaultdict(
            ...
            lambda: defaultdict( # groupby_type
            lambda: defaultdict( # groupby_value
...
def process_type2_features_trigger2_subsuquent_to_trigger_1(agg_defs, history_info, features_info):
    ...
    for level_group, level, filter_type, filter_value, method_agg, col, trigger_start_method0, trigger_start_t0, trigger_start_c0, trigger_end_method1, trigger_end_t1, trigger_end_c1 in agg_defs: # agg_defs[:1]:
...
def inner_generate_features_c_code(history_info, features_src, features_history_init_src, history_names, loop_src):
    ...
    for lg_i, (level_group, level_group_info) in enumerate(history_info.items()):
        ...
        if lg_i == 0:
            features_src.append(f"    if (lg == {level_group}){{{{ // {level_group} => level_group \n")
            features_src.append(loop_src)
            
            features_history_init_src.append(f"    if (lg == {level_group}){{{{ // {level_group} => level_group \n")
        else:
            features_src.append(f"    }}}} else if (lg == {level_group}){{{{ // {level_group} => level_group\n")
            features_src.append(loop_src)
            
            features_history_init_src.append(f"    }}}} else if (lg == {level_group}){{{{ // {level_group} => level_group\n")
        ...
        for level_i, (level, level_info) in enumerate(level_group_info.items()):
            ...
            if level != 'NONE' and level is not None:
                ...
                if is_first_level_if:
                    features_src.append(f"            if (level[i] == {level}){{{{ // {level} => level \n")
                    default_indent_src = '    '
                    is_first_level_if = False
                else:
                    features_src.append(f"            }}}} else if (level[i] == {level}){{{{ // {level} => level \n")
            else:
                assert level_i == 0
                default_indent_src = ''
            ...
            for groupby_type, groupby_type_info in level_info.items():
                ...
                for gb_i, (groupby, groupby_info) in enumerate(groupby_type_info.items()):
                    ...
                    if groupby_type != 'NONE' and groupby_type is not None:
                        features_src.append(f"{default_indent_src}            {if_src} ({gbt_var_src}[i] == {{{gbt_lookup_src}.{groupby_cleaned}}}){{{{ // {gbt_var_src} => gbt_lookup_src \n")
                        indent_src += '    '
                    ...
                    for col, trigger_start_info_dict in groupby_info.items():
                        ...
                        for trigger_start_info, trigger_end_info_dict in trigger_start_info_dict.items():
                            ...
                            if trigger_start_info is not None:
                                ...
                                features_src.append(f"{indent_src}            if (x_index[i] > (history[{{HIST.{trigger_name}}}] + 0.5)){{{{ // {{HIST.{trigger_name}}} => HIST.{trigger_name} \n")
                                trigger_start_indent = "    "
                            ...
                            for trigger_end_info, aggs_info in trigger_end_info_dict.items():
                                ...
                                if trigger_end_info is not None:
                                    ...
                                    features_src.append(f"{indent_src}{trigger_start_indent}            if (x_index[i] < (history[{{HIST.{trigger_name}}}] - 0.5)){{{{ // {{HIST.{trigger_name}}} => HIST.{trigger_name}  \n")
                                    trigger_end_indent = "    "
                                ...
                                if requires_count:
                                    ...
                                    features_src.append(f"{indent}                history[{{HIST.{name}}}] += 1; // {v_src_inc_indexing}, {{HIST.{name}}} => v_src_inc_indexing, HIST.{name} \n")
                                    features_src.append(f"{indent}            }}}}\n")
                                    added_history_names.append(name)

                                    features_history_init_src.append(f"        history[{{HIST.{name}}}] = 0; // {{HIST.{name}}} => HIST.{name} \n")
                                
                                if 'first' in aggs:
                                    
                                    name = get_var_name(level_group, level, groupby_type, groupby, col, trigger_start_info, trigger_end_info, 'first')

                                    features_src.append(f"{indent}            if (isnan(history[{{HIST.{name}}}])){{{{ // {{HIST.{name}}} => HIST.{name} \n")
                                    features_src.append(f"{indent}                history[{{HIST.{name}}}] = {v_src_inc_indexing};\n")
                                    features_src.append(f"{indent}            }}}}\n")
                                    added_history_names.append(name)

                                    features_history_init_src.append(f"        history[{{HIST.{name}}}] = NAN; // {{HIST.{name}}} => HIST.{name} \n")

                                if 'last' in aggs:
                                    
                                    name = get_var_name(level_group, level, groupby_type, groupby, col, trigger_start_info, trigger_end_info, 'last')

                                    features_src.append(f"{indent}            history[{{HIST.{name}}}] = {v_src_inc_indexing}; // {{HIST.{name}}} => HIST.{name} \n")
                                    added_history_names.append(name)
                                ...
                                if 'min' in aggs:
                                    ...
                                    features_src.append(f"{indent}                history[{{HIST.{name}}}] = {v_src_inc_indexing}; // {{HIST.{name}}}, {v_src_inc_indexing} => HIST.{name}, v_src_inc_indexing  \n")
                                    
                                    features_src.append(f"{indent}            }}}}\n")

                                    added_history_names.append(name)
                                    features_history_init_src.append(f"        history[{{HIST.{name}}}] = INFINITY; // {{HIST.{name}}} => HIST.{name} \n")
                                ...
                                if requires_sum:
                                    ...
                                    features_src.append(f"{indent}            if ({conditions_src}){{{{\n")
                                    features_src.append(f"{indent}                history[{{HIST.{name}}}] += {v_src_inc_indexing}; // {{HIST.{name}}} => {{HIST.{name}}}, {v_src_inc_indexing} => v_src_inc_indexing \n")
                                ...
                                if requires_squared_sum:
                                    ...
                                    features_src.append(f"{indent}            if ({conditions_src}){{{{\n")
                                    features_src.append(f"{indent}                history[{{HIST.{name}}}] += pow({v_src_inc_indexing}, 2); // {{HIST.{name}}}, {v_src_inc_indexing} => HIST.{name}, v_src_inc_indexing \n")
                                    features_src.append(f"{indent}            }}}}\n")
                        ...
                        for trigger_start_info, trigger_end_info_dict in trigger_start_info_dict.items():
                            ...
                            for trigger_end_info, aggs_info in trigger_end_info_dict.items():    
                                ...
                                if 'count' in aggs and aggs_info['count']['count'] != 0:
                                    ...
                                    features_src.append(f"        out[{{FN.{count_name}}}] = history[{{HIST.{count_name}}}]; // {{FN.{count_name}}}, {{HIST.{count_name}}} => FN.{count_name}, HIST.{count_name} \n")
                                ...
                                if 'min' in aggs and aggs_info['min']['count'] != 0:
                                    ...
                                    features_src.append(f"        if (history[{{HIST.{count_name}}}] == 0){{{{ // {{HIST.{count_name}}} => HIST.{count_name} \n")
                                    features_src.append(f"            out[{{FN.{name}}}] = NAN; // {{FN.{name}}} => FN.{name}\n")
                                    features_src.append("        }} else {{\n")
                                    features_src.append(f"            out[{{FN.{name}}}] = ((history[{{HIST.{name}}}] == INFINITY) ? NAN : history[{{HIST.{name}}}]); // {{FN.{name}}}, {{HIST.{count_name}}} => FN.{name}, HIST.{count_name} \n")
                                    features_src.append("        }}\n")
                                ...
                                if 'max' in aggs and aggs_info['max']['count'] != 0:
                                    ...
                                    features_src.append(f"            out[{{FN.{name}}}] = NAN; // {{FN.{name}}} => FN.{name}\n")
                                    features_src.append("        }} else {{\n")
                                    features_src.append(f"            temp = ((history[{{HIST.{name}}}] == -INFINITY) ? NAN : history[{{HIST.{name}}}]); // {{HIST.{name}}} => HIST.{name} \n")
                                    features_src.append(f"            out[{{FN.{name}}}] = temp; // {{FN.{name}}} => FN.{name}\n")
                                    features_src.append("        }}\n")
                                ...
                                if 'sum' in aggs and aggs_info['sum']['count'] != 0:
                                    ...
                                    features_src.append(f"            out[{{FN.{name}}}] = NAN; // {{FN.{name}}} => FN.{name} \n")
                                    features_src.append("        }} else {{\n")
                                    features_src.append(f"            out[{{FN.{name}}}] = history[{{HIST.{name}}}]; // {{FN.{name}}}, {{HIST.{name}}} => HIST.{name}, FN.{name} \n")
                                    features_src.append("        }}\n")
                                ...
                                if 'mean' in aggs and aggs_info['mean']['count'] != 0:
                                    ...
                                    features_src.append(f"            out[{{FN.{name}}}] = NAN; // {{FN.{name}}} => FN.{name} \n")
                                    features_src.append("        }} else {{\n")
                                    features_src.append(f"            out[{{FN.{name}}}] = history[{{HIST.{sum_name}}}]/history[{{HIST.{count_name}}}]; // {{FN.{name}}}, {{HIST.{sum_name}}}, {{HIST.{count_name}}} => FN.{name}, HIST.{sum_name}, HIST.{count_name} \n")
                                    features_src.append("        }}\n")
                                ...
                                if 'std' in aggs and aggs_info['std']['count'] != 0:
                                    ...
                                    features_src.append(f"            out[{{FN.{name}}}] = NAN; // {{FN.{name}}} => FN.{name} \n")
                                    features_src.append("        }} else {{\n")
                                    features_src.append(f"            temp_mean = history[{{HIST.{sum_name}}}]/history[{{HIST.{count_name}}}]; // {{HIST.{sum_name}}}, {{HIST.{count_name}}} => HIST.{sum_name}, HIST.{count_name} \n")
                                    ...
                                    features_src.append(f"            )/(history[{{HIST.{count_name}}}]-1)); // {{FN.{name}}}, {{HIST.{squared_sum_name}}}, {{HIST.{count_name}}} => FN.{name}, HIST.{squared_sum_name}, HIST.{count_name} \n")
                                    features_src.append("        }}\n")
                                ...
                                if 'diff' in aggs:
                                    ...
                                    features_src.append(f"            out[{{FN.{name}}}] = history[{{HIST.{end_name}}}] - history[{{HIST.{start_name}}}]; // {{FN.{name}}}, {{HIST.{end_name}}}, {{HIST.{start_name}}} => FN.{name}, HIST.{end_name}, HIST.{start_name} \n")
                                    features_features_names.append(f"{name}")
